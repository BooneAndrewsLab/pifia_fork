...
parser.add_argument( '--backbone', type=str, help='name of the backbone architecture to be used',
                     default='dense_net_121')
parser.add_argument('--width_factor', type=float, help='coefficient to make wider deep loc', default=1.0)

parser.add_argument('--dataset', type=str, help='dataset name (chong or harsha)', default='harsha')

parser.add_argument('--labels_type', type=str,
                    help='type of labels that will be used while training (indiv. proteins, localizations etc.)',
                    default='proteins_mix')
parser.add_argument('--save_prefix', type=str, help='prefix for folder name where model is saved', default='')

parser.add_argument('--dropout_rate', type=float, help='dropout rate to be used in the last layer', default=0.0)
parser.add_argument('--learning_rate', type=float, help='learning rate', default=0.001)
parser.add_argument('--cosine_decay', type=bool, help='use cosine decay schedule', default=False)
parser.add_argument('--do_mask', type=bool, help='using masked cells? Alternatively raw data', default=False)
parser.add_argument('--use_rfp', type=bool, help='using RFP cytosolic channel', default=False)
parser.add_argument('--use_nuc', type=bool, help='using nuclear channel', default=False)
parser.add_argument('--uniform_sampling', type=bool, help='sample equal number of proteins from each class',
                    default=False)
parser.add_argument('--num_epoch', type=int, help='number of epochs to run', required=True)
parser.add_argument('--checkpoint_dir', type=str, help='path to save and look for the checkpoint file',
                    default=None)
parser.add_argument('--saved_weights_dir', type=str, help='path to save the trained weights', default=None)
parser.add_argument('--batch_size', type=int, help='batch size per iteration', default=128)
parser.add_argument('--checkpoint_interval', type=int, help='period to take checkpoints in seconds', default=1800)

parser.add_argument('--dense1_size', type=int, help='number of neurons in 1st dense layer (before feature layer)',
                    default=512)
parser.add_argument('--num_features', type=int, help='number of neurons in 2nd dense layer (feature layer)',
                    default=512)
parser.add_argument('--avg_pool', type=bool, help='do global avg pooling before flatten', default=False)
parser.add_argument('--log_file', type=str, help='specify the location of the output directory, default stdout',
                    default=None)
