...
def decode_img_py_func(file_paths_tuple, file_path_mask, seek_ch, do_mask=True):
    seek_ch_num = tf.get_static_value(seek_ch)
    crop_size = 64
    ...
    for file_path1 in file_paths_tuple:
        ...
        if 'flex' in file_path1:
            file_path_split = file_path1.split('.flex')
            img_path = f'{file_path_split[0]}.flex'
        else:
            file_path_split = file_path1.split('.tiff')
...
def process_path_py_func(file_paths_tuple, file_path_mask, label, seek_ch, num_classes, do_mask=True, onehot=True):
...
def get_dataset_from_lists(filenames_channels_list, filenames_mask, labels, \
    ...
    if num_ch==1:
        dataset = dataset.map(lambda f1,f_mask,l: process_path_py_func([f1],f_mask,l, seek_ch,
                                                                       num_classes, do_mask,onehot), \
                              num_parallel_calls=tf.data.experimental.AUTOTUNE)
    if num_ch==2:
        dataset = dataset.map(lambda f1,f2,f_mask,l: process_path_py_func([f1,f2],f_mask,l,
                                                                          num_classes,do_mask,onehot),
                              num_parallel_calls=tf.data.experimental.AUTOTUNE)
    if num_ch==3:
        dataset = dataset.map(lambda f1,f2,f3,f_mask,l: process_path_py_func([f1,f2,f3],f_mask,l,
                                                                             num_classes,do_mask,onehot),
                              num_parallel_calls=tf.data.experimental.AUTOTUNE)
    if standardize:
